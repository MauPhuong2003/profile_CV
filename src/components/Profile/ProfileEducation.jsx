  ...
  return (
    <section id="education" className="scroll-mt-24 space-y-10">
      <div className="flex items-center gap-3 border-b border-app-border pb-6">
        ...
        <h2 className="font-display text-2xl font-bold tracking-wide text-app-text">{t.education}</h2>
      </div>
      ...
      <div className="space-y-6">
        {(profileData.education || []).map((edu, idx) => (
          <div 
            key={idx} 
            className="p-6 rounded-xl bg-app-card border border-app-border hover:border-app-accent/30 transition-all hover:scale-[1.005] relative overflow-hidden space-y-5 shadow-sm group"
          >
            ...
            <div className="space-y-5 relative z-10">
              <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                <span className="inline-block px-3 py-1 rounded-full text-xs font-bold bg-app-accent/10 text-app-accent border border-app-accent/20 shrink-0">
                  {t.graduated}: {edu.year}
                </span>
              </div>
              <div className="space-y-3">
                <p className="text-xs md:text-sm text-app-muted leading-relaxed tracking-wide">{edu.major}</p>
                {edu.skills && (
                  <div className="flex flex-wrap gap-1.5 pt-1">
                    {edu.skills.split(',').map(s => s.trim()).filter(Boolean).map((skill, index) => (
                      <span 
                        key={index}
